...
parser.add_argument("--test_picture_path", default=r'D:\RAU-Net\inputs\CT\test_256_adaptive_edges', help="path of test datas.")#网络测试输入的图片路径
parser.add_argument("--test_label_path", default=r'D:\RAU-Net\inputs\CT\test_GT_256_adaptive', help="path of test datas.") #网络测试输入的标签路径
parser.add_argument("--image_size", type=int, default=512, help="load image size") #网络输入的尺度
...
parser.add_argument("--snapshots", default=r'D:\RAU-Net\model\Baseline + Att+CLoss_SEA_Unet_7_CT',help="Path of Snapshots") #读取训练好的模型参数的路径
...
def get_write_picture(picture, gen_label, label, height, width): #get_write_picture函数得到网络测试的结果
    ...
    #inv_gen_label_image = cv2.resize(gen_label[0], (width, height))
    ...
    #inv_gen_label_image = inv_gen_label_image / 255.0
    #inv_gen_label_image[inv_gen_label_image > 0.0001] = 1
    #inv_gen_label_image[inv_gen_label_image <= 0.0001] = 0
...
def cv_inv_proc(img): #cv_inv_proc函数将读取图片时归一化的图片还原成原图
    ...
    return img_rgb #返回bgr格式的图像，方便cv2写图像

def get_write_picture( gen_label, height, width): #get_write_picture函数得到网络测试的结果
    #picture_image = cv_inv_proc(picture) #还原输入的图像
    
    gen_label = cv_inv_proc(gen_label[0]) #还原生成的结果
    #label_image = cv_inv_proc(label) #还原读取的标签
    #inv_picture_image = cv2.resize(picture_image, (width, height)) #将输入图像还原到原大小
    #inv_gen_label_image = cv2.resize(gen_label[0], (width, height))
    
    inv_gen_label_image = cv2.resize(gen_label, (width, height)) #将生成的结果还原到原大小
    #inv_label_image = cv2.resize(label_image, (width, height)) #将标签还原到原大小
    #output = np.concatenate((inv_picture_image, inv_gen_label_image, inv_label_image), axis=1) #拼接得到输出结果
...
def main():
    ...
    test_picture = tf.placeholder(tf.float32, shape=[1, args.image_size, args.image_size, 3], name='test_picture') #测试输入的图像
	...
	#读取一张测试图片，一张标签，以及相应的高和宽
        ...
        #batch_picture = np.expand_dims(np.array(batch_picture).astype(np.float32), axis=3) #填充维度
        feed_dict = {test_picture: batch_picture} #构造feed_dict
        gen_label_value = sess.run(gen_label, feed_dict=feed_dict) #得到生成结果
        #plt.imshow(gen_label_value[:,:,0])
        #plt.show()
        #write_image = get_write_picture(picture_resize, gen_label_value, label_resize, picture_height, picture_width) #得到一张需要存的图像
        write_image = get_write_picture(gen_label_value, picture_height, picture_width)
        #write_image[write_image>127]=255
        #write_image[write_image<=127]=0

        #plt.imshow(write_image[:,:,0])
        #plt.show()
